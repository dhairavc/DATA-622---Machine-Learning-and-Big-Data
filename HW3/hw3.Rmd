...
A simple way to dealing with missing values is to conduct "complete case analysis" in that any observations which have a missing value for a given predictor are dropped. However given the small number of observations (614) on this dataset 
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
Given this assumption, we can use the MICE (Multivariate Imputation via Chained Equations) package to impute the missing values throughout the dataset. For each type of predictor variable we will use:  
  
* Numeric Variables: Predictive Mean Matching (ppm)  
* Binary Variables: Logistic Regression (logreg)  
* Factor Variables (3+ levels): Bayesian Polytomous Regression  
