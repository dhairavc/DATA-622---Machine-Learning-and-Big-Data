...
Data conversion
```{r}
loan2 <- loan2[complete.cases(loan2),]
loan2$Gender <- as.factor(loan2$Gender)
loan2$Married <- as.factor(loan2$Married)
loan2$Self_Employed <- as.factor(loan2$Self_Employed)
loan2$Credit_History <- as.factor(loan2$Credit_History)
loan2$Property_Area <- as.factor(loan2$Property_Area)
loan2$Loan_Status <- as.factor(loan2$Loan_Status)
loan2 <- loan2 %>% dplyr::select(-Loan_ID)
```
...
s <- createDataPartition(loan2$Loan_Status, p = .80, list = FALSE, times = 1)
loan2_train <- loan2[s,]
loan2_test <- loan2[-s,]
...
```{r}
loan_dt <- rpart(Loan_Status ~., data = loan2_train, method = "class",
...
```{r}
loan_dt_pred <- predict(loan_dt, loan2_test, type = "class")
loan_dt_table <- table(Actual = loan2_test$Loan_Status, Predicted = loan_dt_pred)
loan_dt_table
...
```{r}
loan_dt_prune_pred <- predict(loan_dt_prune, loan2_test, type = "class")
loan_dt_prune_table <- table(Actual = loan2_test$Loan_Status, Predicted = loan_dt_prune_pred)
loan_dt_prune_table
...
min_tree_var <- tuneRF(x = subset(loan2, select = -Loan_Status), y=loan2$Loan_Status, ntreeTry = 500)
val_opt <- min_tree_var [,"mtry"][which.min(min_tree_var [,"OOBError"])]

loan_rf <- randomForest(Loan_Status ~., data = loan2, importance = TRUE, ntree=500, mtry = val_opt)
loan_rf
...
acc_rf <-  sum(diag(loan_rf$confusion))/sum(loan_rf$confusion)
acc_rf
...
## Model Performancetitle: "Untitled"
